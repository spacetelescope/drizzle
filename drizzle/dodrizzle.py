...
def dodrizzle(insci, input_wcs, inwht,
    ...
    """
    Primary functional routine for performing 'drizzle' operation on one image.
    ...
    performed by the calling routine.  WCS objects need to be based
    on `astropy.wcs`.
    ...
    # This could potentially be computed from the pixmap itself
    # at the position of the input reference position.
    ...
    #
    # Call `dodrizzle` to perform image combination
    #
    _vers, nmiss, nskip = apply_drizzle(insci, inwht, pixmap,
...
def apply_drizzle(insci, inwht, pixmap,
                  outsci, outwht, outcon,
                  expin, in_units, wt_scl,
    ...
    pixmap : 3d array
        Translation lookup table from input pixel position
        to the output pixel frame.
    ...
    pix_ratio : float, optional
        Ratio of output frame pixel scale to the
        undistorted input pixel scale.  Conceptually, the input pixel scale is the
        linear dimension of a side of a pixel in the input image, but it
        is not limited to this and can be set to change how the drizzling
        algorithm operates.
    ...
    Notes
    -----
    Computation of the `pixmap` array prior to calling this function
    requires the following steps:

      1. Determine the size of the input image array
      2. Create X and Y arrays where each element has a value
         corresponding to it's X and Y position, respectively.
      3. Convert those input X and Y input coordinates into
         sky coordinates (RA, Dec) using the input image WCS.
      4. Convert those sky coordinates into output pixel
         coordinates using the output image WCS with all
         X coordinates going to an X array, and Y coordinates
         going to an Y array.
      5. Write out output X,Y arrays as 3D array known as the pixmap.

    The function `calc_pixmap()` in the module
    `drizzle.drizzle.calc_pixmap` serves as an template for computing
    the pixmap using astropy-compatible WCS inputs.
